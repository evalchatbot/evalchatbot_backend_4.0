...
try {
  const genres = await call('/books/genres');
  console.log('Genres:', genres);

  const sess = await call('/user/session/create', 'POST');
  console.log('Session:', sess);

  const chat = await call('/chatbot/ask', 'POST', {
    session_id: sess.session_id,
    question: 'Hi — node test',
    genre: (genres.genres?.[0] || 'history')
  });
